...
const Alert: FC<AlertProps> = ({ component, closeAlert, heading }) => {
  ...
  return (
    <div
      className= " transition-all duration-500 fixed z-50 top-0 left-0 w-full h-full bg-black/30  flex items-center justify-center overflow-hidden scroll-none"
      onClick={(e) => {
    ...
    >
      <div
        className={` bg-theme-2 border border-theme-3 p-5 font-OpenSans flex flex-col ${!isMobile && "rounded-xl"} ${isMobile && "w-screen h-full"
          } `}
      >
        <div
